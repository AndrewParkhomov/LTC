        ...
        if(oldVersion < 1){
            ...
            insertItem(db, "Sphere power", "Spherical correction corrects refractive error of the " +
                    ...
                    "Сфера","Сфера - це оптична сила лінзи, вимірюється в діоптріях, " +
                            "позначається D чи Дптр., застосовується для корекції аномалії рефракції.\n" +
                            "Для далекозорості (міопії) використовують розсіювальні " +
            ...
            insertItem(db, "Real base curve", "The base curve (usually determined from the profile " +
                    ...
                    "Кривизна передней поверхности","Передняя поверхность линзы измеряется сферометром, " +
                            ...
                            "0.001 (плоская передняя поверхность) для изготовления линз с большим " +
                            "отрицательным значением ('минусовых' линз), до 10.5 (примерное значение), " +
                            "для изготовления линз с большим положительным значением ('плюсовых' линз).",
                    "Кривизна передньої поверхні","Передня поверхня лінзи вимірюється сферометром, " +
                            "отримане значення по формулі перераховується в кривизну, виражену " +
                            "в міліметрах або в діоптріях (в цьому додатку). Значення у межах від " +
                            "0.001(плоска передня поверхня) для виготовлення лінз з великим негативним " +
                            "значенням ('мінусових' лінз), до 10.5 (приблизне значення), для виготовлення " +
                            "лінз з великим позитивним значенням ('плюсових' лінз).",R.drawable.front_curve_img);
            ...
            insertItem(db, "Edge thickness", "Thickness of the lens in edge, expressed in millimeters.",
                    ...
                    "Товщина лінзи по краю","Для 'плюсових' лінз мінімальна товщина буде по краю, " +
                            "а для 'мінусових', відповідно, мінімальна товщина буде по центру, " +
