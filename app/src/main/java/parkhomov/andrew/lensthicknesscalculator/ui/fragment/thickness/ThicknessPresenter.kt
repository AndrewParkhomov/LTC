...
constructor(
    ...
    override fun onCalculateBtnClicked(
        ...
        if (spherePower == null) {
            mvpView?.highlightSpherePower(true)
            ...
            if ((spherePower + cylinderPower) <= 0.0) {
                isContinueCalculation = if (centerThickness == null) {
                    ...
                    mvpView?.highlightCenterThickness(false)
                    true
                }
            }
        }
        ...
        if (diameter == null) {
            mvpView?.highlightDiameter(true)
            isContinueCalculation = false
        } else {
            mvpView?.highlightDiameter(false)
        }
        ...
        if (isContinueCalculation) {
            ...
            if (cylinderPower == 0.0) {
                ...
                var etOnCertainAxis = (maxEdgeThickness - edgeThickness) / 90 * axis + edgeThickness
    ...
    private fun handleNoBaseCurveBehaviour(value: Double): Double {
        ...
        mvpView?.setCurrentBaseCurve(tempCurveString)
