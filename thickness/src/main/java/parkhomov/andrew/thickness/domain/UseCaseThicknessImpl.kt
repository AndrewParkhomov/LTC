...
//        _events.value = ShowResultDialog(interactor.calculatedData)
    }
...
//        _events.value = ShowResultDialog(interactor.calculatedData)
    ...
    fun onCalculateBtnClicked(
        ...
        if (spherePower == null) {
//            _events.value = HighlightSpherePower(true)
            isContinueCalculation = false
        } else {
//            _events.value = HighlightSpherePower(false)


            if ((spherePower + cylinderPower) <= 0.0) {
                isContinueCalculation = if (centerThickness == null) {
//                    _events.value = HighlightCenterThickness(true)
                    false
                } else {
//                    _events.value = HighlightCenterThickness(false)
                    true
                }
            }
        }
        ...
        if (diameter == null) {
//            _events.value = HighlightDiameter(true)
            isContinueCalculation = false
        } else {
//            _events.value = HighlightDiameter(false)
        }
        ...
        if (isContinueCalculation) {
            ...
            centerThickness = when {
                ...
                spherePower <= 0 && cylinderPower > 0 && spherePower == 0.0 ->
                    Math.pow(diameter!! / 2, 2.0) * cylinderPower / (2000 * (lensIndex.first - 1)) + edgeThickness
                spherePower <= 0 && cylinderPower > 0 && spherePower + cylinderPower > 0 ->
                    Math.pow(diameter!! / 2, 2.0) * (spherePower + cylinderPower) / (2000 * (lensIndex.first - 1)) + edgeThickness
                spherePower > 0 -> {
                    val tempValue = if (cylinderPower > 0) {
                        spherePower + cylinderPower
                    } else
                        spherePower
                    Math.pow(diameter!! / 2, 2.0) * tempValue / (2000 * (lensIndex.first - 1)) + edgeThickness
                }
...
//        _events.value = BaseCurve(tempCurveString)
