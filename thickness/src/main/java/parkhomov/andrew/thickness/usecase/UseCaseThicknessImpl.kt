...
class UseCaseThicknessImpl(
    ...
    override fun calculateThickness(
        ...
        if (spherePower == null) {
            liveData.value = UseCaseThickness.Result.HighlightSpherePower(true)
            isContinueCalculation = false
        } else {
            liveData.value = UseCaseThickness.Result.HighlightSpherePower(false)

            if (spherePower <= 0.0) {
                isContinueCalculation = if (centerThickness == null) {
                    liveData.value = UseCaseThickness.Result.HighlightCenterThickness(true)
                    false
                } else {
                    liveData.value = UseCaseThickness.Result.HighlightCenterThickness(false)
                    true
                }
            }
        }
        ...
        if (diameter == null) {
            liveData.value = UseCaseThickness.Result.HighlightDiameter(true)
            isContinueCalculation = false
        } else {
            liveData.value = UseCaseThickness.Result.HighlightDiameter(false)
        }
    ...
    private fun handleNoBaseCurveBehaviour(value: Double): Double {
        ...
        liveData.value = UseCaseThickness.Result.SetCurrentBaseCurve(tempCurveString)
